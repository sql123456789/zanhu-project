...
class BaseQATest(CBVTestCase):
    ...
    def setUp(self):
        ...
        # RequestFactory是生成request请求的可以是get也可以是post,直接对request进行处理直接传递给视图代码，不经过url所以这个url可以随便写
        self.request = RequestFactory().get("/fake-url")
        # 把上面创建的request放到我们的request中
...
class TestQuestionListView(BaseQATest):
    ...
    def test_context_data(self):
        ...
        # zip((1,2,3),(4,5,6)) = zip((1,4),(2,4),(3,6))
        # all(True,False,True) == False, all(True,True,True) == True
...
"""
    ...
    TestClient:走Django框架的整个请求响应流程，经过wsgi handler，中间价，url路由
    上下文管理器，返回response，更像是集成测试。特点使用简单，测试一步到为
    测试用例运行慢，依赖中间件，URL路由部分
    
    RequestFactory:生成WSGIRequest使用， 与Django代码无关，单员测试的最佳实践，但是使用难度高
"""
